...
class CricXCrateUI extends HTMLElement {
    ...
    async connectedCallback() {
        // Fetch secure data from the Cloudflare Worker
        try {
            ...
            this._shadowRoot.innerHTML = `
                <div style="display:flex; justify-content:center; align-items:center; height:100vh; background:#000c24; color:#ff2a2a; font-family:sans-serif; text-align:center;">
                    <h2>⚠️ Access Denied<br><span style="font-size:16px; color:#fff;">Unauthorized access. Stream locked to official domain.</span></h2>
                </div>`;
            return;
        }
    ...
    render() {
        this._shadowRoot.innerHTML = `
            ...
            <style>
              /* 
                THE FIX: All variables are now scoped directly to :host 
                THE THEME: UEFA Champions League Inspired (Midnight Navy & Cyan)
              */
              :host { 
                display: block; 
                width: 100%; 
                ...
                --bg-pure: #000c24; 
                --bg-card: rgba(0, 20, 50, 0.82); 
                --border-glass: rgba(0, 195, 255, 0.2);
                --border-glass-bright: rgba(0, 195, 255, 0.45); 
                --heading-dynamic: #ffffff; 
                --text-main: #e6f0fa;
                --text-muted: #78909c; 
                --text-dark: #455a64; 
                --btn-main-bg: #ffffff; 
                --btn-main-text: #000c24;
                --player-shadow: 0 45px 120px -20px rgba(0, 195, 255, 0.25);
              }
              
              /* Light theme retains a sporty daytime feel */
              #cricx-wrapper[data-theme="light"] {
                ...
                --border-glass-bright: rgba(0, 85, 170, 0.4);
                --heading-dynamic: #001a33; 
                --text-main: #001a33; 
                --text-muted: #546e7a; 
                --btn-main-bg: #001a33; 
              ...
              *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
              ...
              .hero-tag { font-family: 'Space Grotesk', sans-serif; color: var(--accent); font-weight: 800; letter-spacing: 2px; }
              ...
              .hero-headline .accent-txt { color: var(--accent); }

              .controls-header-bar { display: flex; flex-direction: column; gap: 10px; margin-bottom: 10px; }
              .channel-search-input { width: 100%; max-width: 320px; background: var(--bg-card); border: 1px solid var(--border-glass-bright); color: var(--text-main); padding: 8px 16px; border-radius: 100px; outline: none; font-family: 'Manrope', sans-serif; }
              ...
              .channel-btn { background: var(--bg-card); color: var(--text-main); border: 1px solid var(--border-glass); padding: 8px 18px; border-radius: 100px; cursor: pointer; font-family: 'Space Grotesk', sans-serif; font-weight: 600; transition: all 0.3s ease; flex-shrink: 0; }
              .channel-btn.active { background: var(--accent); color: #fff; border-color: var(--accent); box-shadow: 0 0 15px var(--accent-glow); }
              ...
              #tg-overlay { position: fixed; inset: 0; z-index: 99998; background: rgba(0, 12, 36, 0.85); backdrop-filter: blur(16px); opacity: 0; pointer-events: none; transition: opacity 0.5s; }
              #tg-overlay.show { opacity: 1; pointer-events: all; }
              #tg-popup { position: fixed; left: 50%; top: 50%; z-index: 99999; width: min(420px, 90vw); background: var(--bg-card); border: 1px solid var(--border-glass-bright); border-radius: 28px; padding: 30px; text-align: center; transform: translate(-50%, -46%) scale(0.92); opacity: 0; pointer-events: none; transition: all 0.5s; box-shadow: 0 20px 50px rgba(0,0,0,0.5); }
              #tg-popup.show { opacity: 1; pointer-events: all; transform: translate(-50%, -50%) scale(1); }
              #tg-join { display: flex; align-items: center; justify-content: center; height: 52px; border-radius: 100px; background: linear-gradient(135deg, #00c3ff 0%, #0055aa 100%); color: #fff; font-family: 'Space Grotesk', sans-serif; font-weight: 900; text-decoration: none; cursor: pointer; margin-bottom: 12px; border: none; box-shadow: 0 10px 20px rgba(0, 195, 255, 0.2); }
              #tg-close { background: transparent; border: 1px solid var(--border-glass-bright); color: var(--text-muted); padding: 10px 20px; border-radius: 100px; cursor: pointer; font-family: 'Manrope', sans-serif; }

              #status { position: fixed; left: 16px; bottom: 16px; z-index: 1000; color: #fff; background: rgba(0, 20, 50, 0.85); border: 1px solid rgba(0, 195, 255, 0.3); border-radius: 10px; padding: 10px 16px; font-size: 12px; }
            ...
            <div id="cricx-wrapper">
                ...
                <div class="container">
                    ...
                    <section class="hero">
                        <div class="hero-tag">UEFA PRO ARENA</div>
                ...
                <div id="tg-popup">
                    <h3 style="color: #fff; font-family: 'Space Grotesk', sans-serif; margin-bottom: 10px;">Join For Free HD Links</h3>
                    <p style="margin-bottom: 20px; color: var(--text-main);">Get instant access to all football feeds, updates, and live streaming links.</p>
    ...
    initWarpField() {
        ...
        const render = () => {
            ...
            stars.forEach(star => {
                ...
                // Uses the UEFA Cyan/Blue mapped to the --primary-rgb variable
                ctx.strokeStyle = isLight ? `rgba(0, 85, 170, ${star.alpha * 0.8})` : `rgba(0, 195, 255, ${star.alpha})`;
                ctx.beginPath();
